...
namespace LINQExercise_2
{
    public class Product
    {
        ...
        public static void Filters()
        {
            ...
            var f1 = ProductList
                .FirstOrDefault(p => p.Category.Equals("Seafood"));
            ...
            var f9 = ProductList
                .FirstOrDefault(p => p.ProductID.Equals(80)).ProductName;

            // listado ordenado por la categoria y luego por el precio

            var f10 = ProductList
                .OrderBy(p => p.Category[0]).ThenBy(p => p.UnitPrice);

            // el producto con mayor cantidad de palabras

            var f11 = ProductList
                .OrderByDescending(p => p.ProductName.Split(' ').Count())
                .FirstOrDefault();
